...
assert.deepEqual(calculateSecurityScore([finding("info", "info"), finding("medium", "medium"), finding("high", "high"), finding("critical", "critical")]), { baseScore: 100, criticalPenalty: 35, highPenalty: 15, mediumPenalty: 6, finalScore: 44 });
assert.equal(calculateSecurityScore([finding("a", "critical"), finding("b", "critical"), finding("c", "critical")]).finalScore, 0);
